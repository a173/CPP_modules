...
bool Form::execute(Bureaucrat const &a) const {
	try {
		if (a.getClass() > _gradeExec)
			throw GradeTooLowException("Уровень Брюрократа " + a.getName() + " cлишком мал для выполнения распоряжения!");
	}
